...
import TreasureAndLootSystem from '#/engine/TreasureAndLootSystem.js';
import WowChatMode from '#/engine/WowChatMode.js';
...
class World {
    ...
    readonly wowChatMode: WowChatMode = new WowChatMode();
    readonly treasureAndLootSystem: TreasureAndLootSystem = new TreasureAndLootSystem();
    ...
    cycle(): void {
        try {
            ...
            // - npc hunt
            this.processWorld();
            this.wowChatMode.tick(this.currentTick);
            this.treasureAndLootSystem.tick(this.currentTick);
        ...
        for (const player of this.players) {
            try {
                ...
                if (this.currentTick % World.AFK_EVENTRATE === 0) {
                    // NODE_RANDOM_EVENT_RATE: 0 = disabled, 100 = normal rate
                    // (normal) 1/12 chance every 5 minutes of setting an afk event state (even distribution 60/5)
                    // (afk) double the chance
                    // TypeScript port note: This percentage scaling would be trivial in Lua/C
                    const eventRate = Environment.NODE_RANDOM_EVENT_RATE;
                    if (eventRate > 0) {
                        const baseChance = player.zonesAfk() ? 0.1666 : 0.0833;
                        const scaledChance = (baseChance * eventRate) / 100;
                        player.afkEventReady = Math.random() < scaledChance;
                    }
                }
    ...
    removeNpc(npc: Npc, duration: number): void {
        this.treasureAndLootSystem.onNpcDeath(npc);
        const zone = this.gameMap.getZone(npc.x, npc.z, npc.level);
