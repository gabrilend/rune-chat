...
export default class Player extends PathingEntity {
    ...
    onLogin() {
        ...
        // sync engine run state from the saved varp so walk/run mode
        // matches what the client displays after login
        this.run = this.vars[VarPlayerType.RUN] ? 1 : 0;
    ...
    updateMovement(): boolean {
        ...
        if (this.stepsTaken > 0) {
            console.log(`[DEBUG updateMovement] run=${this.run} tempRun=${this.tempRun} moveSpeed=${this.moveSpeed} stepsTaken=${this.stepsTaken} runanim=${this.runanim}`);
    ...
    updateEnergy() {
        // NODE_RUN_DRAIN_RATE: 0 = infinite run, 100 = normal drain rate
        // TypeScript port note: This percentage scaling would be trivial in Lua/C
        const drainRate = Environment.NODE_RUN_DRAIN_RATE;
        if (drainRate <= 0) {
            // Infinite run - keep energy at max
        ...
        if (this.stepsTaken < 2) {
            // Recovery is not affected by drain rate setting
            ...
            // Scale loss by drain rate percentage (100 = normal)
            const baseLoss = (67 + (67 * clampWeight) / 64) | 0;
            const loss = ((baseLoss * drainRate) / 100) | 0;
    ...
    tryInteract(allowOpScenery: boolean): boolean {
        ...
        if (opTrigger && (this.target instanceof PathingEntity || allowOpScenery) && this.inOperableDistance(this.target)) {
            ...
            if (target instanceof Loc) {
                World.treasureAndLootSystem.onLocInteraction(this, target as Loc);
            }
    ...
    runScript(script: ScriptState, protect: boolean = false, force: boolean = false) {
        if (!force && protect && (this.protect || this.delayed)) {
            ...
            console.log(`[DEBUG runScript] REJECTED ${script.script.name} protect=${protect} this.protect=${this.protect} this.delayed=${this.delayed} force=${force}`);
            return -1;
        }
