...
const PlayerOps: CommandHandlers = {
    ...
    [ScriptOpcode.P_FINDUID]: state => {
        ...
        if (state.pointerGet(ProtectedActivePlayer[state.intOperand]) && state.activePlayer.uid === uid) {
            ...
            console.log(`[DEBUG P_FINDUID] fast-path success (already protected)`);
            state.pushInt(1);
            return;
        }

        if (!player || !player.canAccess()) {
            console.log(`[DEBUG P_FINDUID] FAILED player=${!!player} canAccess=${player?.canAccess()} protect=${player?.protect} delayed=${player?.delayed} busy=${player?.busy()}`);
            state.pushInt(0);
            return;
        }

        console.log(`[DEBUG P_FINDUID] success via canAccess path`);
        state.activePlayer = player;
        state.pointerAdd(ActivePlayer[state.intOperand]);
    ...
    [ScriptOpcode.P_RUN]: checkedHandler(ProtectedActivePlayer, state => {
        ...
        console.log(`[DEBUG P_RUN] setting run=${value} (was ${state.activePlayer.run})`);
        state.activePlayer.run = value;
