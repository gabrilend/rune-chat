...
export default {
    ...
    // run energy drain rate (0 = infinite run, 100 = normal drain rate)
    // Values > 100 drain faster, values < 100 drain slower
    NODE_RUN_DRAIN_RATE: tryParseInt(process.env.NODE_RUN_DRAIN_RATE, 0),
    // production mode!
    NODE_PRODUCTION: tryParseBoolean(process.env.NODE_PRODUCTION, false),
    // random event rate (0 = disabled, 100 = normal rate)
    // Values > 100 trigger more frequently, values < 100 trigger less frequently
    NODE_RANDOM_EVENT_RATE: tryParseInt(process.env.NODE_RANDOM_EVENT_RATE, 0),
